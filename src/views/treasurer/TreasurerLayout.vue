...
<template>
  <div class="flex min-h-screen bg-gradient-to-br from-emerald-50 via-green-50 to-teal-50 relative overflow-hidden">
    <div class="absolute inset-0 overflow-hidden pointer-events-none">
      <div class="absolute -top-40 -right-40 w-80 h-80 bg-emerald-200 rounded-full mix-blend-multiply filter blur-xl opacity-20 animate-blob"></div>
      <div class="absolute -bottom-40 -left-40 w-80 h-80 bg-teal-200 rounded-full mix-blend-multiply filter blur-xl opacity-20 animate-blob animation-delay-2000"></div>
      <div class="absolute top-40 left-40 w-80 h-80 bg-green-200 rounded-full mix-blend-multiply filter blur-xl opacity-20 animate-blob animation-delay-4000"></div>
    </div>
    ...
    >
      <div class="p-8 border-b border-gray-200/50">
        <div class="flex items-center space-x-3">
          ...
          <div>
            ...
            <p class="text-sm text-gray-500 font-medium">Treasurer Portal</p>
          </div>
        </div>
      </div>
      ...
      <nav class="flex-1 px-6 py-8 space-y-3">
        <router-link 
          ...
          :key="item.name" 
          :to="item.path"
          class="group flex items-center px-4 py-3 rounded-xl transition-all duration-300 text-gray-700 hover:bg-gradient-to-r hover:from-emerald-50 hover:to-teal-50 hover:shadow-md hover:scale-105"
      ...
      <div class="p-6 border-t border-gray-200/50 space-y-4">
        <div class="flex items-center space-x-3 p-3 rounded-xl bg-gray-50/50">
          ...
          <div class="flex-1 min-w-0">
            <p class="text-sm font-medium text-gray-900 truncate">{{ userFirstName || 'Treasurer' }}</p>
        ...
        <div class="flex items-center justify-between px-8 py-4">
          ...
          <div class="flex items-center space-x-6">
            <button class="relative p-2 rounded-xl hover:bg-gray-100 transition-all duration-200 group">
              <BellIcon class="h-6 w-6 text-gray-600 group-hover:text-emerald-600 transition-colors"/>
            ...
            <div class="flex items-center space-x-3 p-2 rounded-xl hover:bg-gray-100 transition-all duration-200 cursor-pointer">
              ...
              <div class="hidden sm:block">
                <p class="text-sm font-medium text-gray-900">{{ userFirstName || 'Treasurer' }}</p>
                <p class="text-xs text-gray-500">{{ userRole || 'Treasurer' }}</p>
              </div>
            </div>
          </div>
